...
void publish_init_kdtree(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes, std::shared_ptr<rclcpp::Node> nh)
{
    ...
    // 2. 포인트클라우드 생성
    PointCloudXYZI::Ptr laserCloudInit(new PointCloudXYZI());
    laserCloudInit->points = ikdtree.PCL_Storage;
    laserCloudInit->width = laserCloudInit->points.size(); // 반드시 일치시킬 것!
    laserCloudInit->height = 1;
    laserCloudInit->is_dense = true;
    ...
    if (laserCloudmsg.header.stamp.sec == 0 && laserCloudmsg.header.stamp.nanosec == 0)
    {
        laserCloudmsg.header.stamp = nh->now();
    }
...
void publish_frame_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes, std::shared_ptr<rclcpp::Node> nh)
{
...
void publish_frame_body(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFull_body, std::shared_ptr<rclcpp::Node> nh)
{
...
void publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr &pubOdomAftMapped,
                      std::shared_ptr<tf2_ros::TransformBroadcaster> &tf_br, std::shared_ptr<rclcpp::Node> nh)
{
    ...
    odomAftMapped.header.stamp = nh->now();
...
int main(int argc, char **argv)
{
    ...
    // } else {
    sub_pcl = nh->create_subscription<sensor_msgs::msg::PointCloud2>(lid_topic, rclcpp::SensorDataQoS().keep_last(1000), standard_pcl_cbk);
    // }
    ...
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubLaserCloudMapReliable;
    rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pubPath;

    if (!odom_only)
    {
        pubLaserCloudFullRes = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_registered", 100000);
        pubLaserCloudFullRes_body = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_registered_body", 100000);
        pubLaserCloudEffect = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_effected", 100000);
        pubLaserCloudMapReliable = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/Laser_map", 100000);
        pubLaserCloudMap = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/Laser_map_reliable", rclcpp::SensorDataQoS());
        pubPath = nh->create_publisher<nav_msgs::msg::Path>("/path", 100000);
    }
    ...
    if (odom_only)
    {
        pubOdomAftMapped = nh->create_publisher<nav_msgs::msg::Odometry>("/odom_corrected", 100000);
    }
    else
    {
        pubOdomAftMapped = nh->create_publisher<nav_msgs::msg::Odometry>("/aft_mapped_to_init", 100000);
    }
    ...
    while (rclcpp::ok())
    {
        ...
        if (sync_packages(Measures))
        {
            ...
            if (!init_map)
            {
                ...
                init_map = true;
                publish_init_kdtree(pubLaserCloudMap, nh);         //(pubLaserCloudFullRes);
                publish_init_kdtree(pubLaserCloudMapReliable, nh); //(pubLaserCloudFullRes);
                continue;
            }
            ...
            if (!use_imu_as_input)
            {
                ...
                for (k = 0; k < time_seq.size(); k++)
                {
                    ...
                    if (publish_odometry_without_downsample)
                    {
                        ...
                        publish_odometry(pubOdomAftMapped, tf_broadcaster, nh);
                        if (runtime_pos_log)
                        {
            ...
            if (scan_pub_en || pcd_save_en)
                publish_frame_world(pubLaserCloudFullRes, nh);
            if (scan_pub_en && scan_body_pub_en)
                publish_frame_body(pubLaserCloudFullRes_body, nh);

            static int laser_map_pub = 0;
            if (++laser_map_pub % 10 == 0)
            {
                publish_init_kdtree(pubLaserCloudMap, nh);
                publish_init_kdtree(pubLaserCloudMapReliable, nh);
            }
